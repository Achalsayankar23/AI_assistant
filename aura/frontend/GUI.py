...
class ChatSection(QWidget):
    def __init__(self):
        ...
        movie = QMovie(GraphicsDirectoryPath("Aura.gif"))
        max_gif_size_W = 480
        max_gif_size_H = 270
    ...
    def LoadMessages(self):
        ...
        with open(TempDirectoryPath('Responses.data'), "r", encoding='utf-8') as file:
            messages = file.read()
        if messages is None or len(messages) <= 1 or str(old_chat_message) == str(messages):
            return
        else:
...
class InitialScreen(QWidget):
    def __init__(self, parent=None):
        ...
        # Set initial mic state from Mic.data
        mic_status = GetMicrophoneStatus()
        self.toggled = mic_status == "True"
    ...
    def toggle_icon(self, event=None):
        ...
        if self.toggled:
            MicButtonClosed()  # Set mic to True
        else:
            MicButtonInitialed()  # Set mic to False
        self.update_mic_icon()
